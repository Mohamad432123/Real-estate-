...
$rabbitConfig = getConfig()->getRabbitMQConfig();

// Configuration from config service
$RABBITMQ_HOST = $rabbitConfig['host'];
$RABBITMQ_PORT = $rabbitConfig['port'];
$RABBITMQ_USER = $rabbitConfig['user'];
$RABBITMQ_PASS = $rabbitConfig['pass'];
$MAPS_REQUEST_QUEUE = $rabbitConfig['maps_request_queue'];
$MAPS_RESPONSE_QUEUE = $rabbitConfig['maps_response_queue'];
...
try {
    ...
    // Declare the queues
    $channel->queue_declare($MAPS_REQUEST_QUEUE, false, true, false, false);
    ...
    $channel->basic_publish($msg, '', $MAPS_REQUEST_QUEUE);

    // Wait for the response with as timeout
    ...
    if ($response) {
        ...
        echo json_encode([
            "status" => "error", 
            "message" => "Timeout: no response received from maps service"
        ]);
    }
